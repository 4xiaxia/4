 ...
 */
export const loadMapSDK = (): Promise<typeof AMap> => {
 ...
 * @returns AMap.Map 实例
 */
export const initMap = (container: HTMLElement, center: [number, number], zoom: number = 15): AMap.Map => {
...
export const addMarker = (map: AMap.Map, position: [number, number], options?: AMap.MarkerOptions): AMap.Marker => {
  ...
  map.add(marker as any); // 使用any来绕过类型检查问题
  return marker;
};
...
export const drawPolyline = (map: AMap.Map, path: [number, number][], options?: AMap.PolylineOptions): AMap.Polyline => {
  ...
  map.add(polyline as any); // 使用any来绕过类型检查问题
  return polyline;
};
